...
impl Core {
    pub(crate) fn new(
        ...
        let attacker_ratio: f64 = env::var("ATTACKER_RATIO")
            ...
            .unwrap_or(0.33);
        let _victim_ratio: f64 = env::var("VICTIM_RATIO")
            .ok()
            .and_then(|s| s.parse().ok())
            .unwrap_or(0.22);
    ...
    fn try_new_block(&mut self, force: bool) -> Option<ExtendedBlock> {
        ...
        // Speculative attack: if enabled and node is attacker, generate multiple candidates by
        // varying transaction order/selection and pick the one with lexicographically largest digest.
        ...
        if self.attack_active && self.is_attacker && self.attack_mode == "speculative" {
            ...
            for i in 0..p_max {
                // Deterministic variation strategy without external deps:
                // - rotate order by i
                // - periodically truncate a small tail to change size
                ...
                let digest = VerifiedBlock::compute_digest(&serialized);
            ...
            info!(
                "Speculative attack: node {} (attacker={}) selected best candidate among {} options, digest={:?}",
    ...
    fn smart_ancestors_to_propose(
        ...
        let included_ancestors = filtered_ancestors;
    ...
    /// FISSURE ATTACK: Pre-process ancestors to exclude victim blocks
    /// This implements the fissure attack by selectively excluding victim blocks from ancestor selection
    /// Optimized: Only exclude victim ancestors that won't break quorum requirements
    fn preprocess_ancestors_for_fissure_attack(
        ...
        for ancestor in ancestors {
            // Check if this ancestor is from a victim node
            ...
            if is_victim {
                ...
                if is_parent_round {
                    // For parent round ancestors, be very careful about quorum
                    // Only exclude if we have a significant margin above quorum threshold
                    // This ensures we don't break quorum when multiple attackers exclude
                    let remaining_stake_after_exclusion = parent_round_stake - ancestor_stake;
                    // Require at least 20% margin above quorum to safely exclude
                    let safe_margin = quorum_threshold + (quorum_threshold / 5);
                    if remaining_stake_after_exclusion >= safe_margin {
                        // Safe to exclude - use full exclusion probability
                        should_exclude = self.should_exclude_victim_block(&ancestor, exclusion_prob);
                    } else if remaining_stake_after_exclusion >= quorum_threshold {
                        // Marginally safe - use reduced probability (50% of base)
                        let safe_exclusion_prob = exclusion_prob * 0.5;
                        should_exclude = self.should_exclude_victim_block(&ancestor, safe_exclusion_prob);
                    } else {
                        // Would break quorum - don't exclude
        ...
        let exclusion_rate = if victim_count_found > 0 {
            (excluded_count as f64 / victim_count_found as f64) * 100.0
        } else {
            0.0
        };

        let metrics = FissureAttackMetrics {
            excluded_count,
            victim_count_found,
            exclusion_rate,
        };
    ...
    fn calculate_exclusion_probability(
        ...
        // Maximum aggressiveness for fissure attack - optimized for high ASR
        // For 13 nodes with 4 attackers and 3 victims:
        // base_prob = 0.5 + 4 / (2*(13-3)) = 0.5 + 4/20 = 0.5 + 0.2 = 0.7
        // With smart quorum-aware exclusion, we can be more aggressive
        let network_factor = if n <= 13.0 {
            7.0  // Very aggressive for 13-node network (quorum-aware exclusion allows this)
        } else {
            6.5  // Strong for larger networks
        };
        
        // No decay - consistent exclusion
        let decay_factor = 1.0;
        
        // Very high exclusion probability - quorum-aware logic prevents breaking consensus
        // Cap at 0.95 for non-critical cases
        let final_prob = (base_prob * network_factor * decay_factor).min(0.95);
        final_prob
    }
