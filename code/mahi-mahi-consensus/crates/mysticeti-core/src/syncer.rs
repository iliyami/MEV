    ...
    fn try_new_block(&mut self) {
        ...
        if attack_mode == "sluggish" || attack_mode == "hybrid_sluggish" {
            let is_attacker = if let Ok(attacker_id) = std::env::var("ATTACKER_ID") {
                self.core.authority() == attacker_id.parse::<crate::types::AuthorityIndex>().unwrap_or(999)
            } else {
                false
            };
            ...
            if is_attacker {
                ...
                // Only delay when we're NOT falling behind
                ...
                tracing::info!(
                    "🐌 SLUGGISH: Node {} delaying {}ms (round_gap={}, multiplier={:.1})",
                    self.core.authority(), delay_ms, round_gap, multiplier
                );
