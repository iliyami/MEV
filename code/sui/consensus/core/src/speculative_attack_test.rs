...
//! Speculative attack ASR test (paper-1 port to upstream Sui).
//!
//! Requires SPECULATIVE_P_MAX to be set. The attacker enumerates up to
//! p_max deterministic transaction reorderings and picks the candidate
//! whose serialized block digest is lexicographically largest.
...
async fn test_speculative_attack_asr_dynamic() {
    ...
    // SAFETY: set before any ConsensusAuthority spawns; single-threaded
    // setup phase, no concurrent readers.
    unsafe {
        ...
        env::set_var("SPECULATIVE_P_MAX", p_max.to_string());
    }

    info!(
        "Speculative attack ASR test: n={num_validators} attackers={num_attacker} victims={num_victim} p_max={p_max}"
    );
