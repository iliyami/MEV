...
export SLUGGISH_TIMEOUT_MULTIPLIER=${SLUGGISH_TIMEOUT_MULTIPLIER:-4.0}
...
# Step 3: Run the test
echo ""
echo "🚀 Step 3: Running 13-node sluggish attack test..."
echo "  Test: test_sluggish_attack_asr_13_nodes"
echo "  Duration: ~30 seconds"
echo "  Expected ASR: 50-60%"
echo ""

# Run the test and capture output
echo "  Executing test..."
TEST_OUTPUT=$(mktemp)
cargo test --release --package mysticeti-core test_sluggish_attack_asr_13_nodes -- --nocapture > "$TEST_OUTPUT" 2>&1
TEST_EXIT_CODE=$?
...
if [ ! -z "$ASR" ]; then
    ...
    DELAYS=$(grep -c "delaying block proposal" "$TEST_OUTPUT" 2>/dev/null || echo "0")
    ...
    if [ "$DELAYS" -gt 0 ]; then
        echo "  Total Delays Applied: $DELAYS"
    fi
    
    echo ""
    echo "🎯 COMPARISON WITH PAPER:"
    echo "========================"
    echo "  Paper's Target: ~82% (for Tusk/Narwhal-Tusk)"
    echo "  Our ASR: $ASR (13 nodes)"
    
    # Extract numeric value for comparison
    ASR_NUM=$(echo "$ASR" | sed 's/%//')
    if (( $(echo "$ASR_NUM >= 40" | bc -l) )); then
        echo "  Status: ✅ SUCCESS - ASR above 40% target!"
        if (( $(echo "$ASR_NUM >= 55" | bc -l) )); then
            echo "  Rating: ⭐⭐⭐ EXCELLENT - ASR ≥ 55%!"
        fi
    else
        echo "  Status: ⚠️  Below 40% target - Review optimization"
    fi
...
# Step 5: Summary
echo ""
echo "🎉 AUTOMATED MYSTICETI SLUGGISH ATTACK COMPLETED!"
echo "================================================="
echo "  Test Output: Saved to temporary file (displayed above)"
echo "  Build log: build.log"
echo ""
echo "🔍 To run manually:"
echo "  export ATTACK_MODE=sluggish"
echo "  export ATTACKER_RATIO=0.308"
echo "  export VICTIM_RATIO=0.231"
echo "  export SLUGGISH_TIMEOUT_MULTIPLIER=4.0"
echo "  export RUST_LOG=info"
echo "  cargo test --release --package mysticeti-core test_sluggish_attack_asr_13_nodes -- --nocapture"
echo ""
