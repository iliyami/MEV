...
# Automated Certification Race Attack Script for Mysticeti
# This script automates the entire process: build, run attack, calculate ASR
# Usage: ./automated_certification_race_attack.sh

echo "🏎️  Automated Certification Race Attack for Mysticeti"
echo "====================================================="
echo "This script will:"
echo "  1. Build the Mysticeti project with attack code"
echo "  2. Run the 13-node network with certification race attack"
...
export ATTACK_MODE=${ATTACK_MODE:-certification_race}
...
echo "  Wave Length: $WAVE_LENGTH"
echo "  Leaders Count: $NUMBER_OF_LEADERS"
echo "  Network Size: 13 nodes"

# Step 3: Run the test
echo ""
echo "🚀 Step 3: Running 13-node certification race attack test..."
...
echo "  Executing test..."
TEST_OUTPUT=$(mktemp)
# Note: Mysticeti uses the same test function for all ASR-based attacks
...
if [ ! -z "$ASR" ]; then
    ...
    RACED_BLOCKS=$(grep -c "CERTIFICATION RACE:" "$TEST_OUTPUT" 2>/dev/null || echo "0")
    COMMITS=$(grep "Collected.*commits" "$TEST_OUTPUT" | tail -1)
    
    if [ ! -z "$COMMITS" ]; then
        echo "  $COMMITS"
    fi
    if [ "$RACED_BLOCKS" -gt 0 ]; then
        echo "  Total Race Events Triggered: $RACED_BLOCKS"
    fi
    
    echo ""
    echo "🎯 EXPERT ANALYSIS:"
    echo "=================="
    echo "  Attack Mode: Certification Race (Hyper-Aggressive)"
    echo "  Protocol Resilience: Baseline ASR comparison"
...
# Step 5: Summary
echo ""
echo "🎉 AUTOMATED CERTIFICATION RACE ATTACK COMPLETED!"
echo "==============================================="
