...
pub struct TestCommitObserver<H = HashSet<TransactionLocator>> {
    ...
    committed_dags: Vec<CommittedSubDag>, // Store committed sub-dags for ASR tracking
...
impl<H: ProcessedTransactionHandler<TransactionLocator>> TestCommitObserver<H> {
    ...
    pub fn committed_dags(&self) -> &Vec<CommittedSubDag> {
        &self.committed_dags
    }
    ...
    for TestCommitObserver<H>
{
    fn handle_commit(
        ...
        for commit in &committed {
            ...
            self.committed_dags.push(commit.clone()); // Store for ASR tracking
