...
impl Core {
    ...
    pub fn spawn(
        ...
        // Log attack configuration at startup
        let attack_mode = std::env::var("ATTACK_MODE").unwrap_or_default();
        if !attack_mode.is_empty() {
            log::info!("🎯 FISSURE STARTUP: Attack mode='{}', Attacker ID={:?}, Victim ID={:?}, Exclusion Prob={:?}", 
                      attack_mode,
                      std::env::var("ATTACKER_ID").ok(),
                      std::env::var("VICTIM_ID").ok(),
                      std::env::var("EXCLUSION_PROBABILITY").ok());
        }
    }
    ...
    async fn process_own_header(&mut self, mut header: Header) -> DagResult<()> {
            ...
            if let Ok(attacker_id_str) = std::env::var("ATTACKER_ID") {
                ...
                if let (Some(attacker_pk), Some(victim_pk)) = (attacker_pk, victim_pk) {
                    if self.name == attacker_pk && header.origin() == victim_pk {
                        ...
                        let exclusion_prob = (base_prob * network_factor).min(0.95);
                        
                        let mut rng = rand::thread_rng();
                        let exclude = rng.gen::<f64>() < exclusion_prob;
                        
                        if exclude {
                            log::info!(
                                "🎯 TIP_EXCLUSION: Attacker {} excluding victim {} tip (height={}) at slot {}",
                                self.name, victim_pk, header.height(), header.height()
                            );
                        }
                        exclude
                    } else {
        ...
        // Only add to tips if not excluded
        if !should_exclude_tip {
    ...
    async fn process_vote(&mut self, vote: Vote, is_loopback: bool) -> DagResult<()> {
        ...
        if !self.sent_cert_to_proposer && (dissemination_ready && consensus_ready) {    
            ...
            // If it's our certificate, send immediately (no delay) for timing attack
            // Or delay for sluggish attack to lag behind in height
            if should_accelerate {
                log::info!("⚡ TIMING_ATTACK: Accelerating our own certificate send (height={})", 
                          cert.height());
            } else if should_delay {
                let multiplier: f64 = std::env::var("SLUGGISH_TIMEOUT_MULTIPLIER")
                    .ok()
                    .and_then(|s| s.parse().ok())
                    .unwrap_or(5.0);
                
                // Delay based on a fixed base delay (e.g., 100ms) multiplied by the multiplier
                let base_delay_ms = 100.0;
                let delay_ms = (base_delay_ms * multiplier) as u64;
                
                log::info!("🐌 SLUGGISH: Delaying our own certificate send by {}ms (multiplier: {:.1}x) at height {} to lag behind", 
                          delay_ms, multiplier, cert.height());
    ...
    fn set_consensus_proposal(&mut self, consensus_message: &mut ConsensusMessage) {
        ...
        match consensus_message { //TODO: Re-factor ConsensusMessages to all have slot/view, option for TC/QC, and a type.
            ConsensusMessage::Prepare {slot, view, tc, qc_ticket: _, proposals} => {
                ...
                if *slot % 100 == 0 {
                    log::info!("🔍 ATTACK CHECK: attack_mode='{}', slot={}, proposals.len()={}", 
                    ...
                    if let Ok(attacker_id_str) = std::env::var("ATTACKER_ID") {
                        ...
                        if let (Some(attacker_pk), Some(victim_pk)) = (attacker_pk, victim_pk) {
                            if self.name == attacker_pk {
                                ...
                                let victim_count = 1; // Single victim
                                ...
                                // Network factor for 13-node network (optimized like Bullshark)
                                let network_factor = if n <= 13.0 { 7.0 } else { 6.5 };
                                let exclusion_prob = (base_prob * network_factor).min(0.95);
                ...
                if attack_mode == "leader_support" {
                    if *slot % 100 == 0 {
                        log::info!("🔍 LEADER_SUPPORT: attack_mode='{}', proposals.len()={}", attack_mode, proposals.len());
                    }
                        ...
                        if let Ok(attacker_id_str) = std::env::var("ATTACKER_ID") {
                            ...
                            if *slot % 100 == 0 {
                                log::info!("🔍 LEADER_SUPPORT: self.name={:?}, attacker_pk={:?}, victim_pk={:?}", 
                                          self.name, attacker_pk, victim_pk);
                            }
                            
                            if let (Some(attacker_pk), Some(victim_pk)) = (attacker_pk, victim_pk) {
                                if self.name == attacker_pk {
                                    log::info!("🎯 LEADER_SUPPORT: Attacker {} activated! Checking proposals...", self.name);
                                
                                // Check if victim is/will be a leader for this slot/view
                                // In Autobahn, leader is determined by (slot + view) % committee.size()
                                let leader_index = (*slot + *view) as usize % authorities_vec.len();
                                let current_leader = authorities_vec.get(leader_index).copied();
                                let is_victim_leader = current_leader == Some(victim_pk);
                                
                                // Also check if victim was leader in recent rounds
                                let prev_leader_index = (*slot + *view).saturating_sub(1) as usize % authorities_vec.len();
                                let prev_leader = authorities_vec.get(prev_leader_index).copied();
                                let was_victim_leader = prev_leader == Some(victim_pk);
                                
                                log::info!("🎯 LEADER_SUPPORT: Slot {} view {} - Current leader: {:?}, Victim: {:?}, Is victim leader: {}", 
                                          slot, view, current_leader, victim_pk, is_victim_leader || was_victim_leader);
                                
                                let mut rng = rand::thread_rng();
                                let mut excluded_count = 0;
                                let initial_count = proposals.len();
                                
                                // Exclude victim proposals, especially if they are/were leaders
                                let victim_exclusion_prob = if is_victim_leader || was_victim_leader {
                                    1.0 // Always exclude if victim is/was leader
                                } else {
                                    exclusion_prob
                                };
                                
                                // Filter out victim proposals
                                proposals.retain(|pk, _| {
                                    if *pk == victim_pk {
                                        let exclude = rng.gen::<f64>() < victim_exclusion_prob;
                                        if exclude {
                                            excluded_count += 1;
                                            log::info!(
                                                "🎯 LEADER_SUPPORT: Attacker {} excluding victim {} proposal (leader={}) at slot {} view {}",
                                                self.name,
                                                victim_pk,
                                                is_victim_leader || was_victim_leader,
                                                slot,
                                                view
                                            );
                                        }
                                ...
                                // Strategic ordering: Convert to Vec, sort, rebuild HashMap
                                // Put attacker proposals LAST (processed FIRST due to LIFO in order_dag)
                                // Put victim proposals FIRST (processed LAST due to LIFO)
                                if !proposals.is_empty() {
                                    let mut proposal_vec: Vec<(PublicKey, Proposal)> = proposals.drain().collect();
                                    
                                    // Sort: attacker last, victim first, others in between
                                    proposal_vec.sort_by(|a, b| {
                                        let a_is_attacker = a.0 == attacker_pk;
                                        let b_is_attacker = b.0 == attacker_pk;
                                        let a_is_victim = a.0 == victim_pk;
                                        let b_is_victim = b.0 == victim_pk;
                                        
                                        match (a_is_attacker, b_is_attacker, a_is_victim, b_is_victim) {
                                            // Attacker should be last (processed first in LIFO)
                                            (true, false, _, _) => std::cmp::Ordering::Greater,
                                            (false, true, _, _) => std::cmp::Ordering::Less,
                                            // Victim should be first (processed last in LIFO)
                                    ...
                                    // Rebuild HashMap (note: HashMap doesn't preserve order, but BTreeMap does)
                                    // Actually, we need to use BTreeMap to preserve order, but proposals is HashMap
                                    // For now, we'll rebuild in reverse order so attacker is added last
                                    // When iterated, HashMap order is non-deterministic, but we can at least
                                    // ensure attacker is in the set
                                    for (pk, proposal) in proposal_vec {
                                        proposals.insert(pk, proposal);
                                    }
                                    
                                    log::info!(
                                        "🎯 LEADER_SUPPORT: Slot {} view {} - Excluded: {}, Final proposals: {}, Victim was leader: {}",
                                        slot, view, excluded_count, proposals.len(), is_victim_leader || was_victim_leader
                                    );
                                }
                            }
                        }
