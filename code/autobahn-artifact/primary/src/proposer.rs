        ...
        loop {
            ...
            tokio::select! {
                ...
                Some(parent) = self.rx_core.recv() => {
                    ...
                    // SLUGGISH ATTACK: Delay processing parent certificates to lag behind in height
                        ...
                        if attacker_id != usize::MAX {
                            ...
                            if let Some(attacker_pk) = attacker_pk {
                                if self.name == attacker_pk {
                                    ...
                                    // Delay based on max_header_delay to lag behind
                                    let base_delay_ms = self.max_header_delay as f64;
                                    let delay_ms = (base_delay_ms * multiplier) as u64;
                                    
                                    info!(
                                        "🐌 SLUGGISH: Attacker {} delaying parent cert processing by {}ms (multiplier: {:.1}x) - staying at height {}",
                                        self.name, delay_ms, multiplier, self.height
                                    );
                                    
                                    // Delay processing parent certificate to lag behind in height
