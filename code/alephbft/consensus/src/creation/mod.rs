...
async fn speculative_attack_select_best_data<H: Hasher, MK: MultiKeychain, DP: DataProvider>(
    preunit: PreUnit<H>,
...
where
    DP::Output: Data,
{
    let p_max: usize = env::var("SPECULATIVE_P_MAX")
        .unwrap_or_else(|_| "50".to_string())
        .parse()
        .unwrap_or(50)
        .min(50); // Cap at 50 to avoid excessive computation
    
    // Get first data
    let first_data_opt = data_provider.get_data().await;
    let first_data = match first_data_opt {
        Some(d) => d,
        None => return None,
    };
    let first_unit = packer.pack(preunit.clone(), Some(first_data.clone()));
    let mut best_hash = first_unit.hash();
    ...
    info!(
        target: LOG_TARGET,
        "Speculative attack: Selected best candidate among {} options, hash={:?}",
        p_max, best_hash
    );
...
async fn create_unit<U: Unit>(
    ...
    loop {
        ...
        process_unit(creator, incoming_parents, is_attacker).await?;
    }
}
...
async fn process_unit<U: Unit>(
    ...
    // This creates a timing environment that can affect when units see enough parents
    // Key insight: Delaying all nodes slightly can create timing advantages for attackers
    ...
    if attack_mode == "sluggish" {
        let processing_delay_ms: u64 = std::env::var("SLUGGISH_PROCESSING_DELAY_MS")
            .unwrap_or_else(|_| "10".to_string())  // Optimal: 10ms processing delay for all nodes
            .parse()
            .unwrap_or(10);
        
        // Apply small delay to all nodes in sluggish mode
        // This creates a timing environment where attackers can benefit from better parent sets
...
async fn keep_processing_units_until<U: Unit>(
    ...
    futures::select! {
        result = keep_processing_units(creator, incoming_parents, is_attacker).fuse() => {
            result?
        },
        _ = until.fuse() => {
...
async fn run_creator<U: Unit, MK: MultiKeychain, DP: DataProvider>(
    ...
    for round in starting_round..max_round {
        ...
        // Check attack mode and attacker status (moved up to use in keep_processing_units_until)
        ...
        if !skip_delay {
            ...
            keep_processing_units_until(&mut creator, incoming_parents, delay, is_attacker).await?;
        }
        
        // SLUGGISH ATTACK: No additional delay after normal wait
        // The processing delay (applied in process_unit) is sufficient
        
        let preunit = create_unit(round, &mut creator, incoming_parents, is_attacker).await?;
        trace!(target: LOG_TARGET, "Created a new preunit {:?} at round {:?}.", preunit, round);
        
        // SPECULATIVE ATTACK: Generate multiple data candidates and pick best
        let data = if attack_mode == "speculative" && is_attacker {
            speculative_attack_select_best_data(preunit.clone(), data_provider, &packer, conf.session_id()).await
        } else {
